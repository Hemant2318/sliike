...
const ViewProducts = () => {
  ...
  const { productCategoryList, permissionsData } = useSelector((state) => ({
  ...
  useEffect(() => {
    if (productPermission?.falsecount !== 3) {
      getProductData(tableData);
      initAPI();
        ...
        value: (
          <Dropdown align="end">
            ...
            <Dropdown.Menu>
              <Dropdown.Item
                href=""
                className="d-flex align-items-center gap-2 mb-1 pointer"
                onClick={() => {
                  navigate(`/products/details/${_id}`);
                }}
              >
